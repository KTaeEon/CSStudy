...
INSERT INTO `User2` VALUES ('a106', '정약용', '010-1234-1003', 33); # 같은 번호여도 UNIQUE 설정이 안되어있어서 중복이 됨

#실습하기 2-2
CREATE TABLE `User3` (
   `uid`  VARCHAR(10) PRIMARY KEY, # 같은 테이블에 2개 이상의 PRIMARY KEY설정 불가능
   `name` VARCHAR(10),
   `hp`   CHAR(13) UNIQUE, # 중복을 막아주는 문자
   `age`  INT
);
...
INSERT INTO `User3` VALUES ('a106', '정약용', '010-1234-1006', 33); # UNIQUE를 설정함으로서 중복을 불가능하게 만듬
...
INSERT INTO `Child` VALUES ('c104', '홍길동', '010-1010-1004', 'p105');
...
#실습하기 2-6
CREATE TABLE `User6` LIKE `User5`; # User5 테이블의 설정값을 복사한 User6 테이블을 만든다.

#실습하기 2-7
INSERT INTO `User6` SELECT * FROM `User5`; # User5 테이블 데이터값을 User6 테이블에 가져오기
INSERT INTO `User6` (`name`, `gender`, `age`, `addr`) SELECT `name`, `gender`, `age`, `addr` FROM `User5`; # User5 테이블 데이터값 중 원하는 것만 가져오기
