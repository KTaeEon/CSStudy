...
#실습 5-3
INSERT INTO `Member` VALUES ('a101', '박혁거세', '010-1234-1001', '부장', 101, 20201119113948);
INSERT INTO `Member` VALUES ('a102', '김유신', '010-1234-1002', '차장', 101, 20201119113948);
INSERT INTO `Member` VALUES ('a103', '김춘추', '010-1234-1003', '사원', 101, 20201119113948);
INSERT INTO `Member` VALUES ('a104', '장보고', '010-1234-1004', '대리', 102, 20201119113948);
INSERT INTO `Member` VALUES ('a105', '강감찬', '010-1234-1005', '과장', 102, 20201119113948);
INSERT INTO `Member` VALUES ('a106', '이성계', '010-1234-1006', '차장', 103, 20201119113948);
INSERT INTO `Member` VALUES ('a107', '정철', '010-1234-1007', '차장', 103, 20201119113948);
INSERT INTO `Member` VALUES ('a108', '이순신', '010-1234-1008', '부장', 104, 20201119113948);
INSERT INTO `Member` VALUES ('a109', '허균', '010-1234-1009', '부장', 104, 20201119113948);
INSERT INTO `Member` VALUES ('a110', '정약용', '010-1234-1010', '사원', 105, 20201119113948);
INSERT INTO `Member` VALUES ('a111', '박지원', '010-1234-1011', '사원', 105, 20201119113948);
...
SELECT SUM(`sale`) AS `매출총합` FROM `Sales` WHERE `year`=2018 AND `month`=1;

#확인2. 2019년 2월달에 5만원 이상 매출에 대한 총합과 평균을 구하시오.
SELECT SUM(`sale`) AS `총합`, 
       AVG(`sale`) AS `평균` 
	 FROM
	    `Sales` WHERE `year`=2019 AND `month`=2 AND `sale` >= 50000;


#확인3. 2020년 전체 매출 가운데 최저, 최고 매출을 구하시오.
SELECT MAX(`sale`) AS `최고`,
       MIN(`sale`) AS `최저`
      FROM
       `Sales` WHERE `year` IN(2020);
...
SELECT `uid`, SUM(`sale`) AS `집계` FROM `Sales` GROUP BY `uid`;
...
#확인1 : 모든 직원의 아이디, 이름, 직급, 부서명을 조회하시오
SELECT a.`uid`, a.`name`, a.`pos`, b.`name`
FROM `Member`     AS a
JOIN `Department` AS b
ON a.`dep`= b.`depNo`;

#확인2 : '김유신' 직원의 2019년도 매출의 합을 조회하시오
SELECT `name`, `year`, SUM(`sale`) AS `합계` 
FROM `Sales` AS a
JOIN `Member`AS b
ON	a.`uid`=b.`uid` 
WHERE a.`uid`='a102' AND `year`=2019;

#확인3 : 2019년 50,000 이상의 매출에 대해 직원별 매출의 합이 100,000원 이상인 직원의 이름, 부서명, 직급, 년도, 매출 합을 초회하시오. 단, 매출 합이 큰 순서부터 정렬
